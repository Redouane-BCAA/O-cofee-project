...
const mainController = {
  homePage: async (req, res) => {
    try {
      const newsCoffees = await dataMapper.getNewsCoffee();
      res.render("home", { newsCoffees });
    } catch (error) {
      res
        .status(500)
    ...
    try {
      ...
      if (categorie) {
        const selectedCategory = categories.find(
          (cat) => cat.name === categorie
        );
        if (selectedCategory) {
          allCoffee = await dataMapper.getCoffeeByCategory(selectedCategory.id);
        } else {
          allCoffee = await dataMapper.getAllCoffee();
        }
      } else {
        allCoffee = await dataMapper.getAllCoffee();
      }
  ...
  productPage: async (req, res) => {
    try {
      const coffee = await dataMapper.getOneCoffee(Number(req.params.id));
      if (!coffee) {
        // Si le produit n'existe pas, on redirige vers la page 404
        return res.status(404).render("404");
      }
      res.render("productPage", { coffee });
    } catch (error) {
      res
        .status(500)
        .send("Erreur côté serveur, les données n'ont pas été remontées");
